...
static void free_instr_list(InstrDef *instrs, size_t count) {
...
static void module_free(Module *m) {
  ...
  free_instr_list(m->instrs, m->instr_count);
  for (size_t i = 0; i < m->func_count; ++i) {
    free(m->funcs[i].name);
    free_instr_list(m->funcs[i].instrs, m->funcs[i].instr_count);
  }
  free(m->imports);
  free(m->consts);
  free(m->funcs);
...
static VmFuncDef *vm_add_func(Module *m, char *name) {
  if (m->func_count == m->func_cap) {
    size_t next = m->func_cap ? m->func_cap * 2 : 4;
    VmFuncDef *p = (VmFuncDef *)realloc(m->funcs, next * sizeof(*p));
    if (!p) return NULL;
    m->funcs = p;
    m->func_cap = next;
  }
...
static int vm_find_func(const Module *m, const char *name) {
  for (size_t i = 0; i < m->func_count; ++i) {
    ...
    if (strcmp(head, "resolve") == 0) {
      ...
      if (target) {
        ...
        if (find_import(m, target) >= 0) {
          form = SRC_FORM_CALL;
        } else if (vm_find_func(m, target) < 0) {
          free(target);
          free(const_name);
          free(const2_name);
          free(head);
...
static int parse_func_form(const char **p, Module *m) {
  char *name = parse_atom(p);
  VmFuncDef *func = NULL;
  if (!name || !name[0] || strcmp(name, "main") == 0 || vm_find_func(m, name) >= 0) {
    free(name);
    return 0;
  }
...
static int parse_module(const char *src, Module *m) {
  ...
  while (1) {
    ...
    else if (strcmp(head, "func") == 0) ok = parse_func_form(&p, m);
...
static int build_label_table_for(const InstrDef *instrs, size_t count, LabelDef **out_labels,
                                 size_t *out_label_count, uint32_t *out_emitted_instrs) {
  LabelDef *labels = (LabelDef *)calloc(count ? count : 1, sizeof(*labels));
  size_t label_count = 0;
  uint32_t emitted = 0;
  if (!labels) return 0;
  for (size_t i = 0; i < count; ++i) {
...
static int compile_instr_buf(const Module *m, const InstrDef *instrs, size_t count,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (in->form == SRC_FORM_CONST_U64 || in->form == SRC_FORM_ADD_U64 ||
      ...
      uint8_t op = in->form == SRC_FORM_CONST_U64 ? OP_CONST_U64 :
                   ...
                   OP_GE_I64;
      emit_instr(out_instrs, op, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_EXPECT) {
      emit_instr(out_instrs, OP_EXPECT_U64, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_CONST_BOOL) {
      emit_instr(out_instrs, OP_CONST_BOOL, (uint32_t)in->imm, 0);
      continue;
    }
    if (in->form == SRC_FORM_NULL_PTR) {
      emit_instr(out_instrs, OP_NULL_PTR, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_ADD_PTR) {
      emit_instr(out_instrs, OP_ADD_PTR, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_SUB_PTR) {
      emit_instr(out_instrs, OP_SUB_PTR, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_PTR_TO_U64) {
      emit_instr(out_instrs, OP_PTR_TO_U64, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_U64_TO_PTR) {
      emit_instr(out_instrs, OP_U64_TO_PTR, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_CONST_PTR) {
      ...
      emit_instr(out_instrs, OP_CONST_PTR, (uint32_t)const_idx, 0);
      continue;
    }
    if (in->form == SRC_FORM_LOAD_U8) {
      emit_instr(out_instrs, OP_LOAD_U8, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_LOAD_U16) {
      emit_instr(out_instrs, OP_LOAD_U16, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_LOAD_U32) {
      emit_instr(out_instrs, OP_LOAD_U32, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_STORE_U8) {
      emit_instr(out_instrs, OP_STORE_U8, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_STORE_U16) {
      emit_instr(out_instrs, OP_STORE_U16, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_STORE_U32) {
      emit_instr(out_instrs, OP_STORE_U32, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_IS_NULL_PTR) {
      emit_instr(out_instrs, OP_IS_NULL_PTR, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_IS_NONNULL_PTR) {
      emit_instr(out_instrs, OP_IS_NONNULL_PTR, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_NOT_BOOL) {
      emit_instr(out_instrs, OP_NOT_BOOL, 0, 0);
      continue;
    }
    if (in->form == SRC_FORM_AND_BOOL) {
      emit_instr(out_instrs, OP_AND_BOOL, (uint32_t)in->imm, 0);
      continue;
    }
    if (in->form == SRC_FORM_OR_BOOL) {
      emit_instr(out_instrs, OP_OR_BOOL, (uint32_t)in->imm, 0);
      continue;
    }
    if (in->form == SRC_FORM_EXPECT_I64) {
      emit_instr(out_instrs, OP_EXPECT_I64, (uint32_t)(in->imm & 0xffffffffu),
                 (uint32_t)(in->imm >> 32));
      continue;
    }
    if (in->form == SRC_FORM_EXPECT_BOOL) {
      emit_instr(out_instrs, OP_EXPECT_BOOL, (uint32_t)in->imm, 0);
      continue;
    }
    if (in->form == SRC_FORM_EXPECT_PTR) {
      emit_instr(out_instrs, OP_EXPECT_PTR, (uint32_t)in->imm, 0);
      continue;
    }
    if (in->form == SRC_FORM_BRANCH) {
      ...
      emit_instr(out_instrs, OP_BRANCH_BOOL, labels[label_idx].pc, 0);
      continue;
    }
    if (in->form == SRC_FORM_CALL_FUNC) {
      int func_idx = vm_find_func(m, in->import_name);
      if (func_idx < 0) return 0;
      emit_instr(out_instrs, OP_CALL_FUNC, (uint32_t)func_idx, 0);
      continue;
    }
    ...
    if (in->form == SRC_FORM_RESOLVE) {
      emit_instr(out_instrs, OP_RESOLVE_IMPORT, (uint32_t)import_idx, 0);
      continue;
    }
    ...
    if (sig == SIG_I32_VOID) {
      ...
      emit_instr(out_instrs, OP_CALL_IMPORT_VOID, (uint32_t)import_idx, 0);
    } else if (sig == SIG_I32_I32) {
      int32_t imm = 0;
      if (!in->const_name || in->const2_name || !parse_i32_atom(in->const_name, &imm)) {
        return 0;
      }
      emit_instr(out_instrs, OP_CALL_IMPORT_IMM, (uint32_t)import_idx, (uint32_t)imm);
    } else if (sig == SIG_U64_PTR || sig == SIG_I32_PTR) {
      int const_idx = in->const_name ? find_const(m, in->const_name) : -1;
      if (const_idx < 0 || in->const2_name) {
        return 0;
      }
      emit_instr(out_instrs, OP_CALL_IMPORT_CONST, (uint32_t)import_idx, (uint32_t)const_idx);
...
static unsigned char *compile_module(const Module *m, size_t *out_n) {
  ...
  if (!build_label_table_for(m->instrs, m->instr_count, &labels, &label_count, &emitted_instrs)) {
    free(imports.data);
    free(consts.data);
    free(strings.data);
    return NULL;
  ...
  if (!compile_instr_buf(m, m->instrs, m->instr_count, labels, label_count, &instrs)) {
    free(imports.data);
    free(consts.data);
    free(instrs.data);
    free(strings.data);
  ...
  Buf func_entries = {0};
  uint32_t total_emitted = emitted_instrs + 1;
  for (size_t fi = 0; fi < m->func_count; ++fi) {
    const VmFuncDef *func = &m->funcs[fi];
    LabelDef *flabels = NULL;
    size_t flabel_count = 0;
    uint32_t func_emitted = 0;
    uint32_t start_pc = total_emitted;
    if (!build_label_table_for(func->instrs, func->instr_count, &flabels, &flabel_count,
                               &func_emitted)) {
      free(imports.data);
      free(consts.data);
      free(instrs.data);
      free(strings.data);
      free(func_entries.data);
      return NULL;
    }
    if (!compile_instr_buf(m, func->instrs, func->instr_count, flabels, flabel_count, &instrs)) {
      free(flabels);
      free(imports.data);
      free(consts.data);
      free(instrs.data);
    ...
    free(flabels);
    buf_put32(&func_entries, start_pc);
    buf_put32(&func_entries, func_emitted);
    buf_put32(&func_entries, 0);
    total_emitted += func_emitted;
  }
  emitted_instrs = total_emitted - 1;

  unsigned char magic[8] = OUTPUT_MAGIC_INIT;
  buf_put(&out, magic, 8);
  buf_put32(&out, 1);
  buf_put32(&out, (uint32_t)m->func_count);
  ...
  buf_put(&out, func_entries.data, func_entries.len);
