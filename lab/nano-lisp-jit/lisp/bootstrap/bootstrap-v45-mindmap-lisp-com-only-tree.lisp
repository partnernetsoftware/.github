; Wave35: lisp-com-only 活图终局树.
(bootstrap
  (file-size "lab/nano-lisp-jit/v4.5/mindmap-frontier-v45-lisp-com-only.json")
  (file-size "lab/nano-lisp-jit/release/nano-lisp.com")
  (file-size "lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-converge-lisp-only.lisp")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.lisp_com.output_named" "1")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.mindmap.lisp_com_only.coupled" "1"))
