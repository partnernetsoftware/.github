; Wave51 goal: v4.5 规划面终局签收（≠ 154KB 物理 DONE）.
(bootstrap
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.goal.onion_tdd_tree_mindmap.100" "1")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.v45.endgame_honest_rollup_continue.100" "1")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.v45.lispjit_codegen_dedicated_continue.100" "1")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.rollup.extension_waves_all" "1")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.honest.endgame_remaining" "1")
  (results-min "lab/nano-lisp-jit/.build/v45-entry.evidence" "v45.v45.v45_terminal_complete.100" "1"))
