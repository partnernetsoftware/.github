...
expect_inspect_failure() {
  ...
  local out=""
  local status=0
  out=$("$runner" "$mode" "$path" 2>&1 >/dev/null) || status=$?
  printf '%s.status=%s\n' "$mode" "$status"
  printf '%s.stderr=%s\n' "$mode" "$out"
  if [ "$status" -ne 2 ]; then
    return 1
...
(bootstrap
  ...
  (inspect-elf64-exe "$BUILD_DIR/bootstrap-aot-const-ptr-load-u8-code.elf")
  ...
  (inspect-elf64-obj "$BUILD_DIR/bootstrap-aot-const-ptr-callee.o")
  ...
  (inspect-elf64-exe "$BUILD_DIR/bootstrap-aot-const-ptr-cross-linked")
...
run_case "nano-jit-run-const-ptr-load-only-rodata" "$BUILD_DIR/nano-jit.com" run-expect-exit "$CONST_PTR_LOAD_ONLY_LINK_EXE" 1
...
run_case "nano-jit-inspect-reject-bad-symtab-order" expect_inspect_failure "$BUILD_DIR/nano-jit.com" inspect-elf64-obj "$CONST_PTR_BAD_SYMTAB_ORDER_OBJ" "bad_symtab_order"
