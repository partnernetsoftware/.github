...
Slice **0** 已 **100%** 签收（`nano-cc compile` CLI + `bootstrap-v35-nano-cc-hello` + 负向 exit 2）。切片 **1–6** 在并行分支上同时开工；**合并**仍按洋葱 TDD 与依赖波次，见 [`../ROADMAP.md`](../ROADMAP.md) v3.5 mindmap。

## 并行轨道

| Track | Slice | 焦点 | 硬依赖 | 状态 |
|-------|-------|------|--------|------|
| A | 1 C-subset 前端 | `add-parse`、token→IR | slice 0 | **kickoff ~5%** |
| B | 2 x86_64 发射 | object + `link-elf64-exe` | slice 1 IR 接口 | **kickoff ~5%**（等 A 冻结 dump） |
| C | 3 `build-slice` 切换 | `build-slice-codegen`、`role=nano-cc` | slice 1+2 至少 add 路径 | **0%** blocked |
| D | 4 aarch64 nano-cc | cross + qemu | slice 2 模式可抄 | **kickoff ~5%** |
| E | 5 gen3 自举 | `bootstrap-v35-selfhost-gen3` | slice 3 或 genesis-pin 过渡 | **0%** blocked |
| F | 6 Genesis 收缩 | pin + `NANO_REGENESIS` | slice 5 日常零 `cc` | **0%** blocked |
