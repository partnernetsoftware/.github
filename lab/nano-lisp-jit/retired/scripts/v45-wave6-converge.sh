...
w3_ok=0
if run_plan lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-wave6-w3-minimal-probe.lisp; then
  w3_ok=1
  echo "v45-wave6-converge=ok w3_minimal_probe=1"
fi
if [ -x "$NEXT" ]; then
  if "${GEN[@]}" "$NEXT" run-bootstrap-plan \
      lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-onion-lisp-only.lisp >/dev/null 2>&1; then
