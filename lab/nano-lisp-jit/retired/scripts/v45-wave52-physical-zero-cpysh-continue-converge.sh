#!/usr/bin/env bash
# Wave52: physical-zero-cpysh-continue — v4.5 物理续推 · 快 seed（默认）.
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
EV="$ROOT/lab/nano-lisp-jit/.build/v45-entry.evidence"
COM="$ROOT/lab/nano-lisp-jit/.build/nano-jit/nano-jit.com"
NEXT_FULL="$ROOT/lab/nano-lisp-jit/release/v45-selfhost-next.com"
PZC_FRONTIER="$ROOT/lab/nano-lisp-jit/v4.5/mindmap-frontier-v45-physical-zero-cpysh-continue.json"
...
touch "$EV"
if [ ! -x "$COM" ]; then
  echo "v45-wave52-physical-zero-cpysh-continue-converge=skip missing_com"
  exit 0
fi
...
echo "v45-wave52-physical-zero-cpysh-continue-converge=begin"
...
if [ -x "$NEXT_FULL" ]; then
  for spec in \
    "pzi:lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-physical-zero-cpysh-inventory.lisp" \
    "pza:lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-physical-zero-cpysh-honest-anchor.lisp" \
    "cdzc:lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-converge-daily-zero-cpysh.lisp" \
    "szcm:lab/nano-lisp-jit/lisp/bootstrap/bootstrap-v45-selfhost-zero-cpysh-matrix.lisp"; do
    name=${spec%%:*}
    plan=${spec#*:}
    ( next_plan_ok "$plan" && echo "v45-wave52=ok next_pzc $name" ) \
      || { echo "v45-wave52=fail next_pzc $name"; exit 1; } &
    bpids+=($!)
  done
else
  broad_ok=0
...
host_ok=1
hpids=()
for p in physical-zero-cpysh-inventory physical-zero-cpysh-honest-anchor \
  converge-daily-zero-cpysh selfhost-zero-cpysh-matrix; do
...
if [ "$host_ok" = 1 ]; then
  echo "v45.physical.zero_cpysh_inventory=1" >>"$EV"
  echo "v45.honest.zero_cpysh_remaining=1" >>"$EV"
  echo "v45.converge.daily_zero_cpysh=1" >>"$EV"
  echo "v45.selfhost.zero_cpysh_matrix=1" >>"$EV"
  echo "v45.mindmap.physical_zero_cpysh_continue.coupled=1" >>"$EV"
fi
if [ "$broad_ok" = 1 ] && [ -x "$NEXT_FULL" ]; then
  echo "v45.runner.selfhost_next_pzc=1" >>"$EV"
fi

for p in mindmap-physical-zero-cpysh-continue-tree wave52-diffuse-global wave52-rollup \
  goal-v45-physical-zero-cpysh-continue-100; do
  run_plan "$p" && echo "v45-wave52=ok plan=$p" \
    || { echo "v45-wave52=fail plan=$p"; fail=$((fail + 1)); }
done

python3 - <<'PY' "$PZC_FRONTIER" || fail=$((fail + 1))
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
...
print(f"v45-wave52=ok physical_zero_cpysh_continue_frontier {done}/{total}")
PY
...
if [ "$fail" = 0 ] && [ "$host_ok" = 1 ] && [ "$broad_ok" = 1 ]; then
  ...
  echo "v45-wave52-physical-zero-cpysh-continue-converge=done fail=0"
  exit 0
fi
echo "v45-wave52-physical-zero-cpysh-continue-converge=done fail=$fail host=$host_ok broad=$broad_ok"
exit 1
