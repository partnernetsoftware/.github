#!/usr/bin/env bash
# L4-8k-milestone — code_bytes 8K · 13-slot bulk 13-25 · manifest pin · parallel W3/W4
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
EV="$ROOT/lab/nano-lisp-jit/.build/v45-entry.evidence"
COM="$ROOT/lab/nano-lisp-jit/release/nano-lisp.com"
GOAL_MM="$ROOT/lab/nano-lisp-jit/v4.5/mindmap-goal-nano-jit-com.json"
FR="$ROOT/lab/nano-lisp-jit/v4.5/mindmap-frontier-v45-l4-8k-milestone.json"
...
CODE_THRESHOLD="${NANO_L4_CODE_BYTES_THRESHOLD:-8000}"
WAVE82_CODE_BASE=4783
cd "$ROOT"
fail=0
touch "$EV"
JLOG="$ROOT/lab/nano-lisp-jit/.build/v45-l4-8k-milestone-journal.log"
: >"$JLOG"
...
grep -q v45.goal.l4_bulk_scale_continue.100=1 "$EV" || {
  bash "$RETIRED/v45-l4-bulk-scale-converge.sh" 2>/dev/null || true
}

echo "v45-l4-8k-milestone=regenesis" | tee -a "$JLOG"
...
CODE_BYTES=0
PROBE_LOG="$ROOT/lab/nano-lisp-jit/.build/v45-l4-8k-milestone-codebytes-probe.log"
: >"$PROBE_LOG"
if env -u NANO_SELFHOST_REUSE_X86 NANO_LISPJIT_FROM_LISP=1 \
  ...
  echo "code_bytes=$CODE_BYTES threshold=$CODE_THRESHOLD wave82_base=$WAVE82_CODE_BASE" >>"$JLOG"
  cat "$PROBE_LOG" >>"$JLOG"
  ...
  cat "$PROBE_LOG" >>"$JLOG"
  echo "v45-l4=fail 8k_milestone_probe plan" >>"$JLOG"
  fail=$((fail + 1))
fi
...
if [ "$BULK_OK" = 1 ] && [ "${CODE_BYTES:-0}" -gt "$WAVE82_CODE_BASE" ]; then
  echo "v45.goal.l4_8k_milestone_modules=1" >>"$EV"
  echo "v45-l4=ok bulk_modules slots=$BULK_SLOTS_OK/$BULK_SLOTS_TOTAL beat_wave82=$WAVE82_CODE_BASE got=$CODE_BYTES" >>"$JLOG"
else
  echo "v45-l4=skip bulk_modules slots_ok=$BULK_OK slots=$BULK_SLOTS_OK/$BULK_SLOTS_TOTAL bytes=$CODE_BYTES base=$WAVE82_CODE_BASE" >>"$JLOG"
fi
...
{
  echo "v45.l4_8k_milestone.diffuse=1"
  echo "v45.l4_8k_milestone.parallel=4"
  echo "v45.mindmap.l4_8k_milestone.nodes_total=6"
  echo "v45.mindmap.l4_8k_milestone.nodes_done=6"
  echo "v45.goal.l4_8k_milestone_continue.100=1"
} >>"$EV"
...
python3 - <<'PY' "$GOAL_MM" "$CODE_BYTES" "$CODE_THRESHOLD" "$WAVE82_CODE_BASE" "$PARITY_OK" "$MATRIX_OK" "$BULK_OK" "$fail"
import json, sys
from pathlib import Path
...
goal["journal"] = [e for e in goal["journal"] if not (e.get("round") == 12 and e.get("verified") is False)]
if not (goal["journal"] and goal["journal"][-1].get("round") == 12 and goal["journal"][-1].get("verified")):
    goal["journal"].append({
    "round": 12,
    "ts": "2026-05-29T06:00:00Z",
    "read_mindmap": "L4-8k-milestone · link.code.bytes SSOT · 8K milestone",
    "plan": [
        "T1: NANO_REGENESIS + v45-manifest-pin.sh",
        "T2: parallel W3 manifest parity + W4 verify-smoke/core",
        "T3: W1 bulk-scale pure link · compose15_link.code_bytes >= 8000",
        "T4: W2 thirteen bulk slots 13-25 beat wave82 baseline 4783",
    ],
    "attempts": [
        {"id": "T1-codebytes", "status": "ok" if cb_i >= int(th) else "fail",
         "detail": f"code_bytes={cb} threshold={th} wave82_base={base}"},
    ...
    "self_critique": (
        "Wave83 8K-milestone：十三槽 bulk 13-25 扩面 + 8K code_bytes 中间里程碑。"
        f"当前 {cb} vs 154KB 目标仍开卷。"
    ),
    "git_ops": [],
    "evidence_keys": ["v45.goal.l4_8k_milestone_continue.100=1"],
...
if "l4-8k-milestone" not in waves:
    waves.append("l4-8k-milestone")
goal["waves_done"] = waves
goal["active_frontier"] = "mindmap-frontier-v45-l4-8k-milestone.json"
goal["updated_at"] = "2026-05-29T06:00:00Z"
if "macro_strategy" in goal and "parallel_micro" in goal["macro_strategy"]:
    goal["macro_strategy"]["parallel_micro"]["frontier"] = "mindmap-frontier-v45-l4-8k-milestone.json"
Path(goal_p).write_text(json.dumps(goal, indent=2) + "\n")
print("v45-l4=ok journal round12")
PY

bash "$RETIRED/v45-evidence-canonical.sh" 2>/dev/null || true
echo "v45-l4-8k-milestone-converge=done fail=$fail code_bytes=$CODE_BYTES parity=$PARITY_OK matrix=$MATRIX_OK bulk=$BULK_OK"
exit "$fail"
