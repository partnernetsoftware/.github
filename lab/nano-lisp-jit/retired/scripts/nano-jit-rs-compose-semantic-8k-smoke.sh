#!/usr/bin/env bash
# nanolisp compose-15 semantic-8k smoke — tu-main-8k + semantic core/mf → exit 42 + code parity.
set -euo pipefail
...
[ -x "$RS" ] || { echo "nano-jit-rs-compose-semantic-8k-smoke=fail no_binary"; exit 1; }
[ -x "$COM" ] || { echo "nano-jit-rs-compose-semantic-8k-smoke=fail no_legacy_com"; exit 1; }
TMP="$ROOT/lab/nano-lisp-jit/.build/nano-jit-rs-compose-semantic-8k-smoke"
mkdir -p "$TMP"

# link order — profile compose-15link-semantic (Wave94)
SPECS=(
  "$SEM/tu-main-8k.lisp:nano_tu_main:main"
...
LINKED="$TMP/semantic-8k-linked.elf"
...
echo "$log" | grep -q 'run-expect-exit.ok=1' || {
  echo "nano-jit-rs-compose-semantic-8k-smoke=fail run"
  echo "$log"
  exit 1
}

rs_bytes=$("$RS" link-elf64-exe "$TMP/semantic-8k-linked2.elf" nano_tu_main "${RS_OBJS[@]}" 2>&1 | sed -n 's/^link.code.bytes=//p')
com_bytes=$("$COM" link-elf64-exe "$TMP/semantic-8k-com.elf" nano_tu_main "${COM_OBJS[@]}" 2>&1 | sed -n 's/^link.code.bytes=//p')
[ "$rs_bytes" = "$com_bytes" ] || {
  echo "nano-jit-rs-compose-semantic-8k-smoke=fail code_bytes rs=$rs_bytes com=$com_bytes"
  exit 1
}
[ "${rs_bytes:-0}" -ge "$THRESHOLD" ] || {
  echo "nano-jit-rs-compose-semantic-8k-smoke=fail below_threshold bytes=$rs_bytes threshold=$THRESHOLD"
  exit 1
}

echo "nano-jit-rs-compose-semantic-8k-smoke=ok link.code.bytes=$rs_bytes threshold=$THRESHOLD"
