...
#define NANO_CC_CANONICAL_ADD_LISP "lab/nano-lisp-jit/lisp/core/nano-jit-slice-add.lisp"
...
static int nano_cc_compile_obj_via_companion_lisp(const char *src_path, const char *out_path) {
  char lisp_path[4096];
  int a = 0;
  int b = 0;
  size_t n = 0;
...
static int cmd_nano_cc_compile_obj(const char *src_path, const char *out_path) {
  size_t n = 0;
  unsigned char *src = read_file(src_path, &n);
  int code = 0;
  int rc;
  ...
  if (nano_cc_target_is_aarch64()) {
    free(src);
    fprintf(stderr, "nano-cc=obj_arch_unsupported arch=aarch64 path=%s\n", src_path);
    return 2;
  }
  if (nano_cc_parse_main_return((const char *)src, n, &code)) {
    free(src);
    if (!emit_elf64_obj_ret_file(out_path, "main", (uint32_t)code)) {
      fprintf(stderr, "nano-cc=emit_obj_fail path=%s\n", out_path);
      return 3;
    }
    printf("nano-cc.source=%s\n", src_path);
    printf("nano-cc.output=%s\n", out_path);
    printf("nano-cc.obj.symbol=main\n");
    printf("nano-cc.obj.mode=relocatable\n");
    printf("nano-cc.exit_code=%d\n", code);
    return 0;
  }
  free(src);
  rc = nano_cc_compile_obj_via_companion_lisp(src_path, out_path);
  if (rc == 0) return 0;
  if (rc == -1) {
