#!/usr/bin/env python3
"""Generate v4 wave batch samples. Usage: gen-v4-wave-batch.py 68 73"""
from __future__ import annotations
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SAMPLES = ROOT / "samples"

WAVES = {
    68: dict(
        title="ir-table-recap",
        a=46,
        b_track="irtable",
        c_track="chain",
        b_files=[
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "samples/bootstrap-v4-wave56-host-reduce-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
        ],
        c_files=[
            "samples/bootstrap-v4-wave55-diffusion.lisp",
            "archive/v4/slices/SLICE56.md",
            "samples/bootstrap-v4-wave56-diffusion.lisp",
        ],
        b_test="samples/bootstrap-v4-wave56-irtable-tick.lisp",
        c_test="archive/v4/slices/SLICE56.md",
    ),
    69: dict(
        title="build-graph-deep",
        a=47,
        b_track="buildgraph",
        c_track="gates",
        b_files=[
            "samples/bootstrap-v4-build-graph-wave27.lisp",
            "samples/bootstrap-v4-wave44-build-graph-tick.lisp",
            "samples/bootstrap-v4-wave31-diffusion.lisp",
        ],
        c_files=[
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "archive/v4/slices/SLICE31.md",
            "samples/bootstrap-v4-wave44-diffusion.lisp",
        ],
        b_test="samples/bootstrap-v4-build-graph-wave27.lisp",
        c_test="archive/v4/slices/SLICE31.md",
    ),
    70: dict(
        title="host-reduce-chain",
        a=48,
        b_track="hostreduce",
        c_track="wave33",
        b_files=[
            "samples/bootstrap-v4-wave32-diffusion.lisp",
            "samples/bootstrap-v4-wave32-host-reduce-tick.lisp",
            "archive/v4/slices/SLICE32.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave33-diffusion.lisp",
            "samples/bootstrap-v4-build-graph-wave27.lisp",
            "archive/v4/slices/SLICE33.md",
        ],
        b_test="samples/bootstrap-v4-wave32-host-reduce-tick.lisp",
        c_test="archive/v4/slices/SLICE33.md",
    ),
    71: dict(
        title="plan-contract-recap",
        a=49,
        b_track="contract",
        c_track="manifest",
        b_files=[
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "samples/bootstrap-v4-wave34-diffusion.lisp",
            "archive/v4/slices/SLICE34.md",
        ],
        c_files=[
            "samples/v4-plan-manifest-v1.lisp",
            "samples/bootstrap-v4-wave29-diffusion.lisp",
            "archive/v4/slices/SLICE29.md",
        ],
        b_test="samples/bootstrap-v4-plan-contract-tick.lisp",
        c_test="archive/v4/slices/SLICE34.md",
    ),
    72: dict(
        title="evidence-matrix-deep",
        a=50,
        b_track="evmatrix",
        c_track="resume",
        b_files=[
            "samples/bootstrap-v4-wave48-manifest-tick.lisp",
            "samples/bootstrap-v4-wave48-contract-tick.lisp",
            "archive/v4/slices/SLICE48.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave60-evidence-tick.lisp",
            "samples/bootstrap-v4-squad-resume-tick.lisp",
            "archive/v4/slices/SLICE60.md",
        ],
        b_test="samples/bootstrap-v4-wave48-manifest-tick.lisp",
        c_test="archive/v4/slices/SLICE48.md",
    ),
    73: dict(
        title="four-track-milestone",
        a=51,
        b_track="fourtrack",
        c_track="onion",
        b_files=[
            "samples/bootstrap-v4-wave57-fourtrack-tick.lisp",
            "samples/bootstrap-v4-wave57-contract-tick.lisp",
            "archive/v4/slices/SLICE57.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave58-onion-tick.lisp",
            "samples/bootstrap-v4-wave58-postv4-tick.lisp",
            "archive/v4/slices/SLICE58.md",
        ],
        b_test="samples/bootstrap-v4-wave57-fourtrack-tick.lisp",
        c_test="archive/v4/slices/SLICE58.md",
    ),
    74: dict(
        title="lisp-runner-scout",
        a=52,
        b_track="runner",
        c_track="lisponly",
        b_files=[
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "samples/bootstrap-v4-lisp-only-tick.lisp",
            "v4/LISP-ONLY.md",
        ],
        b_test="samples/bootstrap-v4-wave62-runner-tick.lisp",
        c_test="v4/LISP-ONLY.md",
    ),
    75: dict(
        title="codegen-emit-deep",
        a=53,
        b_track="emit",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="archive/v4/slices/SLICE61.md",
    ),
    76: dict(
        title="mindmap-autonomous",
        a=54,
        b_track="mindmap",
        c_track="eval",
        b_files=[
            "samples/bootstrap-v4-wave52-mindmap-tick.lisp",
            "v4/MINDMAP.md",
            "v4/PARALLEL.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave52-progress-tick.lisp",
            "v4/EVAL.md",
            "v4/PROGRESS.md",
        ],
        b_test="v4/MINDMAP.md",
        c_test="v4/EVAL.md",
    ),
    77: dict(
        title="squad-commander-resume",
        a=55,
        b_track="commander",
        c_track="resume",
        b_files=[
            "samples/bootstrap-v4-wave66-commander-tick.lisp",
            "samples/bootstrap-v4-squad-commander-tick.lisp",
            "archive/v4/slices/SLICE66.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave72-resume-tick.lisp",
            "samples/bootstrap-v4-squad-resume-tick.lisp",
            "archive/v4/slices/SLICE72.md",
        ],
        b_test="samples/bootstrap-v4-squad-commander-tick.lisp",
        c_test="archive/v4/slices/SLICE72.md",
    ),
    78: dict(
        title="build-graph-onion",
        a=56,
        b_track="buildgraph",
        c_track="gates",
        b_files=[
            "samples/bootstrap-v4-wave69-buildgraph-tick.lisp",
            "samples/bootstrap-v4-build-graph-wave27.lisp",
            "archive/v4/slices/SLICE69.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave69-gates-tick.lisp",
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "archive/v4/slices/SLICE31.md",
        ],
        b_test="samples/bootstrap-v4-build-graph-wave27.lisp",
        c_test="archive/v4/slices/SLICE31.md",
    ),
    79: dict(
        title="longrun-milestone",
        a=57,
        b_track="longrun",
        c_track="parallel",
        b_files=[
            "archive/v4/factory-docs/LONG-RUN-TODO.md",
            "v4/DECISION.md",
            "v4/REFLECTION.md",
        ],
        c_files=[
            "v4/PARALLEL.md",
            "samples/bootstrap-v4-wave55-autonomous-tick.lisp",
            "archive/v4/slices/SLICE55.md",
        ],
        b_test="archive/v4/factory-docs/LONG-RUN-TODO.md",
        c_test="v4/PARALLEL.md",
    ),
    80: dict(
        title="ir-table-scout-deep",
        a=58,
        b_track="irtable",
        c_track="hostreduce",
        b_files=[
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "samples/bootstrap-v4-wave68-irtable-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
        ],
        c_files=[
            "samples/bootstrap-v4-wave32-diffusion.lisp",
            "samples/bootstrap-v4-wave70-hostreduce-tick.lisp",
            "archive/v4/slices/SLICE32.md",
        ],
        b_test="samples/bootstrap-v4-wave68-irtable-tick.lisp",
        c_test="samples/bootstrap-v4-wave70-hostreduce-tick.lisp",
    ),
    81: dict(
        title="emit-manifest-chain",
        a=59,
        b_track="emit",
        c_track="manifest",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        c_files=[
            "samples/v4-plan-manifest-v1.lisp",
            "samples/bootstrap-v4-wave48-manifest-tick.lisp",
            "archive/v4/slices/SLICE48.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-plan-manifest-v1.lisp",
    ),
    82: dict(
        title="four-track-autonomous",
        a=60,
        b_track="fourtrack",
        c_track="contract",
        b_files=[
            "samples/bootstrap-v4-wave57-fourtrack-tick.lisp",
            "samples/bootstrap-v4-wave73-fourtrack-tick.lisp",
            "archive/v4/slices/SLICE57.md",
        ],
        c_files=[
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "samples/bootstrap-v4-wave71-contract-tick.lisp",
            "archive/v4/slices/SLICE34.md",
        ],
        b_test="samples/bootstrap-v4-wave73-fourtrack-tick.lisp",
        c_test="samples/bootstrap-v4-wave71-contract-tick.lisp",
    ),
    83: dict(
        title="reflection-resume-deep",
        a=61,
        b_track="reflection",
        c_track="resume",
        b_files=[
            "samples/bootstrap-v4-wave63-mindmap-tick.lisp",
            "v4/REFLECTION.md",
            "archive/v4/slices/SLICE63.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave72-resume-tick.lisp",
            "samples/bootstrap-v4-squad-resume-tick.lisp",
            "archive/v4/slices/SLICE72.md",
        ],
        b_test="samples/bootstrap-v4-wave63-mindmap-tick.lisp",
        c_test="samples/bootstrap-v4-wave72-resume-tick.lisp",
    ),
    84: dict(
        title="lisp-only-terminal",
        a=62,
        b_track="lisponly",
        c_track="terminal",
        b_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "v4/LISP-ONLY.md",
            "archive/v4/slices/SLICE64.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave67-terminal-tick.lisp",
            "samples/bootstrap-v4-terminal-build-evidence.lisp",
            "archive/v4/slices/SLICE58.md",
        ],
        b_test="samples/bootstrap-v4-wave64-lisponly-tick.lisp",
        c_test="samples/bootstrap-v4-wave67-terminal-tick.lisp",
    ),
    85: dict(
        title="codegen-emit-milestone",
        a=63,
        b_track="codegen",
        c_track="emit",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="samples/bootstrap-v4-wave61-codegen-tick.lisp",
        c_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
    ),
    86: dict(
        title="runner-plan-deep",
        a=64,
        b_track="runner",
        c_track="plan",
        b_files=[
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        c_files=[
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "archive/v4/slices/SLICE34.md",
            "v4/DECISION.md",
        ],
        b_test="samples/bootstrap-v4-wave62-runner-tick.lisp",
        c_test="archive/v4/slices/SLICE34.md",
    ),
    87: dict(
        title="squad-evidence-chain",
        a=65,
        b_track="assess",
        c_track="evmatrix",
        b_files=[
            "samples/bootstrap-v4-wave42-assess-bundle-tick.lisp",
            "samples/bootstrap-v4-squad-assess-once.lisp",
            "archive/v4/slices/SLICE42.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave48-manifest-tick.lisp",
            "samples/bootstrap-v4-evidence-matrix.lisp",
            "archive/v4/slices/SLICE48.md",
        ],
        b_test="samples/bootstrap-v4-squad-assess-once.lisp",
        c_test="archive/v4/slices/SLICE48.md",
    ),
    88: dict(
        title="onion-mindmap-close",
        a=66,
        b_track="onion",
        c_track="mindmap",
        b_files=[
            "samples/bootstrap-v4-wave58-onion-tick.lisp",
            "v4/MINDMAP.md",
            "v4/PARALLEL.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave52-mindmap-tick.lisp",
            "v4/EVAL.md",
            "v4/PROGRESS.md",
        ],
        b_test="v4/MINDMAP.md",
        c_test="v4/EVAL.md",
    ),
    89: dict(
        title="host-reduce-recap",
        a=67,
        b_track="hostreduce",
        c_track="buildgraph",
        b_files=[
            "samples/bootstrap-v4-wave56-host-reduce-tick.lisp",
            "samples/bootstrap-v4-wave70-hostreduce-tick.lisp",
            "archive/v4/slices/SLICE32.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave33-diffusion.lisp",
            "samples/bootstrap-v4-build-graph-wave27.lisp",
            "archive/v4/slices/SLICE33.md",
        ],
        b_test="samples/bootstrap-v4-wave56-host-reduce-tick.lisp",
        c_test="archive/v4/slices/SLICE33.md",
    ),
    90: dict(
        title="ir-table-chain",
        a=68,
        b_track="irtable",
        c_track="chain",
        b_files=[
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "samples/bootstrap-v4-wave80-irtable-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
        ],
        c_files=[
            "samples/bootstrap-v4-wave55-diffusion.lisp",
            "archive/v4/slices/SLICE56.md",
            "samples/bootstrap-v4-wave56-diffusion.lisp",
        ],
        b_test="samples/v4-ir-table-v1.lisp",
        c_test="archive/v4/slices/SLICE56.md",
    ),
    91: dict(
        title="four-track-recap",
        a=69,
        b_track="fourtrack",
        c_track="contract",
        b_files=[
            "samples/bootstrap-v4-wave73-fourtrack-tick.lisp",
            "samples/bootstrap-v4-wave82-fourtrack-tick.lisp",
            "archive/v4/slices/SLICE73.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave71-contract-tick.lisp",
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "archive/v4/slices/SLICE71.md",
        ],
        b_test="samples/bootstrap-v4-wave73-fourtrack-tick.lisp",
        c_test="archive/v4/slices/SLICE71.md",
    ),
    92: dict(
        title="longrun-skill-anchor",
        a=70,
        b_track="longrun",
        c_track="parallel",
        b_files=[
            "archive/v4/factory-docs/LONG-RUN-TODO.md",
            "v4/longrun-state.json",
            "v4/MINDMAP.md",
        ],
        c_files=[
            "v4/PARALLEL.md",
            "samples/bootstrap-v4-wave55-autonomous-tick.lisp",
            "v4/MINDMAP.md",
        ],
        b_test="archive/v4/factory-docs/LONG-RUN-TODO.md",
        c_test="v4/MINDMAP.md",
    ),
    93: dict(
        title="squad-commander-chain",
        a=71,
        b_track="commander",
        c_track="assess",
        b_files=[
            "samples/bootstrap-v4-squad-commander-tick.lisp",
            "samples/bootstrap-v4-wave66-commander-tick.lisp",
            "archive/v4/slices/SLICE66.md",
        ],
        c_files=[
            "samples/bootstrap-v4-squad-assess-once.lisp",
            "samples/bootstrap-v4-wave42-assess-bundle-tick.lisp",
            "archive/v4/slices/SLICE42.md",
        ],
        b_test="samples/bootstrap-v4-squad-commander-tick.lisp",
        c_test="samples/bootstrap-v4-squad-assess-once.lisp",
    ),
    94: dict(
        title="codegen-table-deep",
        a=72,
        b_track="codegen",
        c_track="emit",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="samples/v4-ir-table-v1.lisp",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    95: dict(
        title="runner-lisp-only",
        a=73,
        b_track="runner",
        c_track="lisponly",
        b_files=[
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "samples/bootstrap-v4-lisp-only-tick.lisp",
            "v4/LISP-ONLY.md",
        ],
        b_test="v4/LISP-ONLY.md",
        c_test="v4/DECISION.md",
    ),
    96: dict(
        title="build-graph-recap",
        a=74,
        b_track="buildgraph",
        c_track="gates",
        b_files=[
            "samples/bootstrap-v4-build-graph-wave27.lisp",
            "samples/bootstrap-v4-wave69-buildgraph-tick.lisp",
            "archive/v4/slices/SLICE69.md",
        ],
        c_files=[
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "samples/bootstrap-v4-wave69-gates-tick.lisp",
            "archive/v4/slices/SLICE31.md",
        ],
        b_test="samples/bootstrap-v4-build-graph-wave27.lisp",
        c_test="archive/v4/slices/SLICE31.md",
    ),
    97: dict(
        title="mindmap-eval-close",
        a=75,
        b_track="mindmap",
        c_track="eval",
        b_files=[
            "v4/MINDMAP.md",
            "samples/bootstrap-v4-wave52-mindmap-tick.lisp",
            "v4/PARALLEL.md",
        ],
        c_files=[
            "v4/EVAL.md",
            "v4/PROGRESS.md",
            "samples/bootstrap-v4-wave52-progress-tick.lisp",
        ],
        b_test="v4/MINDMAP.md",
        c_test="v4/EVAL.md",
    ),
    98: dict(
        title="evidence-matrix-recap",
        a=76,
        b_track="evmatrix",
        c_track="resume",
        b_files=[
            "samples/bootstrap-v4-evidence-matrix.lisp",
            "samples/bootstrap-v4-wave72-evmatrix-tick.lisp",
            "archive/v4/slices/SLICE72.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave72-resume-tick.lisp",
            "samples/bootstrap-v4-squad-resume-tick.lisp",
            "archive/v4/slices/SLICE60.md",
        ],
        b_test="samples/bootstrap-v4-evidence-matrix.lisp",
        c_test="archive/v4/slices/SLICE72.md",
    ),
    99: dict(
        title="terminal-onion",
        a=77,
        b_track="terminal",
        c_track="onion",
        b_files=[
            "samples/bootstrap-v4-wave67-terminal-tick.lisp",
            "samples/bootstrap-v4-terminal-build-evidence.lisp",
            "archive/v4/slices/SLICE58.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave58-onion-tick.lisp",
            "samples/bootstrap-v4-wave40-onion-tick.lisp",
            "archive/v4/slices/SLICE40.md",
        ],
        b_test="samples/bootstrap-v4-terminal-build-evidence.lisp",
        c_test="archive/v4/slices/SLICE58.md",
    ),
    100: dict(
        title="ir-words-scout",
        a=78,
        b_track="irwords",
        c_track="irtable",
        b_files=[
            "samples/bootstrap-v4-wave44-diffusion.lisp",
            "samples/v4-ir-words-v2.txt",
            "archive/v4/slices/SLICE44.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE56.md",
        ],
        b_test="samples/v4-ir-words-v2.txt",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    101: dict(
        title="post-v4-anchor",
        a=79,
        b_track="postv4",
        c_track="scoped",
        b_files=[
            "samples/bootstrap-v4-wave49-postv4-tick.lisp",
            "samples/bootstrap-v4-wave58-postv4-tick.lisp",
            "v4/POST-V4.md",
        ],
        c_files=[
            "v4/COMPLETE-SCOPED.md",
            "samples/bootstrap-v4-wave46-kickoff-tick.lisp",
            "archive/v4/slices/SLICE46.md",
        ],
        b_test="v4/POST-V4.md",
        c_test="v4/COMPLETE-SCOPED.md",
    ),
    102: dict(
        title="autonomous-milestone-2",
        a=80,
        b_track="autonomous",
        c_track="longrun",
        b_files=[
            "samples/bootstrap-v4-wave55-autonomous-tick.lisp",
            "v4/DECISION.md",
            "archive/v4/slices/SLICE55.md",
        ],
        c_files=[
            "archive/v4/factory-docs/LONG-RUN-TODO.md",
            "v4/longrun-state.json",
            "v4/MINDMAP.md",
        ],
        b_test="v4/DECISION.md",
        c_test="archive/v4/factory-docs/LONG-RUN-TODO.md",
    ),
    103: dict(
        title="orchestration-bundle",
        a=81,
        b_track="orchestration",
        c_track="dispatch",
        b_files=[
            "samples/bootstrap-v4-squad-orchestration-bundle.lisp",
            "samples/bootstrap-v4-wave36-orchestration-tick.lisp",
            "archive/v4/slices/SLICE36.md",
        ],
        c_files=[
            "samples/bootstrap-v4-squad-dispatch.lisp",
            "samples/bootstrap-v4-squad-run-loop-once.lisp",
            "v4/SQUAD.md",
        ],
        b_test="samples/bootstrap-v4-squad-orchestration-bundle.lisp",
        c_test="v4/SQUAD.md",
    ),
    104: dict(
        title="runner-scout-resume",
        a=82,
        b_track="runner",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "archive/v4/slices/SLICE62.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="samples/bootstrap-v4-wave62-runner-tick.lisp",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    105: dict(
        title="emit-lowering-chain",
        a=83,
        b_track="emit",
        c_track="runner",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/LISP-ONLY.md",
    ),
    106: dict(
        title="codegen-table-resume",
        a=84,
        b_track="codegen",
        c_track="emit",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        b_test="samples/v4-ir-table-v1.lisp",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    107: dict(
        title="runner-plan-v2",
        a=85,
        b_track="runner",
        c_track="lisponly",
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "archive/v4/slices/SLICE86.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "samples/bootstrap-v4-lisp-only-tick.lisp",
            "v4/LISP-ONLY.md",
        ],
        b_test="samples/bootstrap-v4-wave86-runner-tick.lisp",
        c_test="v4/LISP-ONLY.md",
    ),
    108: dict(
        title="emit-manifest-v2",
        a=86,
        b_track="emit",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    109: dict(
        title="codegen-milestone-3",
        a=87,
        b_track="codegen",
        c_track="emit",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        b_test="samples/bootstrap-v4-wave61-codegen-tick.lisp",
        c_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
    ),
    110: dict(
        title="runner-lisp-bridge",
        a=88,
        b_track="runner",
        c_track="irtable",
        b_files=[
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "archive/v4/slices/SLICE56.md",
        ],
        c_files=[
            "samples/v4-ir-table-v1.lisp",
            "samples/v4-ir-words-v2.txt",
            "archive/v4/slices/SLICE44.md",
        ],
        b_test="samples/bootstrap-v4-wave62-runner-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    111: dict(
        title="turbo-skill-evidence",
        a=89,
        b_track="longrun",
        c_track="reflection",
        b_files=[
            "archive/v4/factory-docs/LONG-RUN-TODO.md",
            "v4/longrun-state.json",
            "v4/MINDMAP.md",
        ],
        c_files=[
            "v4/REFLECTION.md",
            "v4/PROGRESS.md",
            "v4/EVAL.md",
        ],
        b_test="archive/v4/factory-docs/LONG-RUN-TODO.md",
        c_test="v4/REFLECTION.md",
    ),
    112: dict(
        title="terminal-runner-codegen",
        a=90,
        b_track="runner",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave95-runner-tick.lisp",
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "archive/v4/slices/SLICE95.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="samples/bootstrap-v4-wave95-runner-tick.lisp",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    113: dict(
        title="emit-result-obs",
        a=91,
        b_track="emit",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE113.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="archive/v4/slices/SLICE113.md",
    ),
    114: dict(
        title="runner-emit-bridge",
        a=92,
        b_track="runner",
        c_track="emit",
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "archive/v4/slices/SLICE86.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        b_test="samples/bootstrap-v4-wave86-runner-tick.lisp",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    115: dict(
        title="codegen-lowering-resume",
        a=93,
        b_track="codegen",
        c_track="emit",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE8.md",
            "archive/v4/slices/SLICE9.md",
        ],
        b_test="samples/v4-ir-table-v1.lisp",
        c_test="archive/v4/slices/SLICE8.md",
    ),
    116: dict(
        title="runner-plan-v3",
        a=94,
        b_track="runner",
        c_track="plan",
        b_files=[
            "samples/bootstrap-v4-wave95-runner-tick.lisp",
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "archive/v4/slices/SLICE95.md",
        ],
        c_files=[
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "archive/v4/slices/SLICE34.md",
            "v4/DECISION.md",
        ],
        b_test="samples/bootstrap-v4-wave95-runner-tick.lisp",
        c_test="archive/v4/slices/SLICE34.md",
    ),
    117: dict(
        title="emit-ir-bridge",
        a=95,
        b_track="emit",
        c_track="irtable",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/v4-ir-words-v2.txt",
            "archive/v4/slices/SLICE44.md",
        ],
        c_files=[
            "samples/v4-ir-table-v1.lisp",
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "archive/v4/slices/SLICE56.md",
        ],
        b_test="samples/v4-ir-words-v2.txt",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    118: dict(
        title="codegen-manifest-v3",
        a=96,
        b_track="codegen",
        c_track="gates",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        c_files=[
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "samples/bootstrap-v4-wave69-gates-tick.lisp",
            "archive/v4/slices/SLICE31.md",
        ],
        b_test="samples/bootstrap-v4-codegen-kickoff.lisp",
        c_test="archive/v4/slices/SLICE31.md",
    ),
    119: dict(
        title="runner-lisp-deep-v2",
        a=97,
        b_track="runner",
        c_track="lisponly",
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "samples/bootstrap-v4-lisp-only-tick.lisp",
            "v4/LISP-ONLY.md",
        ],
        b_test="v4/LISP-ONLY.md",
        c_test="v4/DECISION.md",
    ),
    120: dict(
        title="emit-terminal-chain",
        a=98,
        b_track="emit",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    121: dict(
        title="runner-codegen-close",
        a=99,
        b_track="runner",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave95-runner-tick.lisp",
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "archive/v4/slices/SLICE95.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="samples/bootstrap-v4-wave95-runner-tick.lisp",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    122: dict(
        title="emit-verify-chain",
        a=100,
        b_track="emit",
        c_track="codegen",
        extra_grep=[
            "aarch64.emit.add.result=",
            "aarch64.emit.add.operands=",
        ],
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE113.md",
            "archive/v4/factory-docs/DEV-AGENTS-TEAM.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="archive/v4/slices/SLICE113.md",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    123: dict(
        title="runner-emit-v4",
        a=101,
        b_track="runner",
        c_track="emit",
        extra_grep=["aarch64.emit.add.result="],
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "samples/bootstrap-v4-wave62-runner-tick.lisp",
            "archive/v4/slices/SLICE86.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE113.md",
            "archive/v4/slices/SLICE65.md",
        ],
        b_test="samples/bootstrap-v4-wave86-runner-tick.lisp",
        c_test="archive/v4/slices/SLICE113.md",
    ),
    124: dict(
        title="codegen-lowering-v4",
        a=102,
        b_track="codegen",
        c_track="emit",
        extra_grep=["aarch64.emit.add.operands="],
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE8.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE9.md",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/v4-ir-table-v1.lisp",
        c_test="archive/v4/slices/SLICE8.md",
    ),
    125: dict(
        title="emit-ir-v4",
        a=103,
        b_track="emit",
        c_track="irtable",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/v4-ir-words-v2.txt",
            "archive/v4/slices/SLICE44.md",
        ],
        c_files=[
            "samples/v4-ir-table-v1.lisp",
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "archive/v4/slices/SLICE56.md",
        ],
        b_test="samples/v4-ir-words-v2.txt",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    126: dict(
        title="runner-plan-v4",
        a=104,
        b_track="runner",
        c_track="plan",
        b_files=[
            "samples/bootstrap-v4-wave95-runner-tick.lisp",
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "archive/v4/slices/SLICE34.md",
        ],
        c_files=[
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "v4/DECISION.md",
            "v4/LISP-ONLY.md",
        ],
        b_test="samples/bootstrap-v4-wave95-runner-tick.lisp",
        c_test="archive/v4/slices/SLICE34.md",
    ),
    127: dict(
        title="codegen-gates-v4",
        a=105,
        b_track="codegen",
        c_track="gates",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "archive/v4/slices/SLICE31.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave69-gates-tick.lisp",
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "archive/v4/slices/SLICE31.md",
        ],
        b_test="samples/bootstrap-v4-codegen-kickoff.lisp",
        c_test="archive/v4/slices/SLICE31.md",
    ),
    128: dict(
        title="emit-terminal-v4",
        a=106,
        b_track="emit",
        c_track="codegen",
        extra_grep=["aarch64.emit.add.result="],
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE65.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    129: dict(
        title="runner-lisp-v4",
        a=107,
        b_track="runner",
        c_track="lisponly",
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "samples/bootstrap-v4-lisp-only-tick.lisp",
            "v4/LISP-ONLY.md",
        ],
        b_test="v4/LISP-ONLY.md",
        c_test="v4/DECISION.md",
    ),
    130: dict(
        title="dev-agents-milestone",
        a=108,
        b_track="longrun",
        c_track="team",
        b_files=[
            "archive/v4/factory-docs/DEV-AGENTS-TEAM.md",
            "archive/v4/factory-docs/LONG-RUN-TODO.md",
            "v4/longrun-state.json",
        ],
        c_files=[
            "v4/REFLECTION.md",
            "v4/PROGRESS.md",
            "v4/EVAL.md",
        ],
        b_test="archive/v4/factory-docs/DEV-AGENTS-TEAM.md",
        c_test="v4/REFLECTION.md",
    ),
    131: dict(
        title="emit-grep-v2",
        a=109,
        b_track="emit",
        c_track="codegen",
        extra_grep=["aarch64.emit.add.result=", "aarch64.emit.add.operands="],
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "archive/v4/slices/SLICE113.md", "archive/v4/factory-docs/DEV-AGENTS-TEAM.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/bootstrap-v4-codegen-kickoff.lisp", "archive/v4/slices/SLICE41.md"],
        b_test="archive/v4/slices/SLICE113.md",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    132: dict(
        title="runner-codegen-main",
        a=110,
        b_track="runner",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave95-runner-tick.lisp", "samples/bootstrap-v4-wave62-runner-tick.lisp", "archive/v4/slices/SLICE95.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave95-runner-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    133: dict(
        title="codegen-emit-main",
        a=111,
        b_track="codegen",
        c_track="emit",
        extra_grep=["aarch64.emit.add.result="],
        b_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE8.md", "archive/v4/slices/SLICE9.md"],
        c_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "archive/v4/slices/SLICE113.md", "archive/v4/slices/SLICE65.md"],
        b_test="archive/v4/slices/SLICE8.md",
        c_test="archive/v4/slices/SLICE113.md",
    ),
    134: dict(
        title="runner-plan-main",
        a=112,
        b_track="runner",
        c_track="plan",
        b_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/bootstrap-v4-plan-contract-tick.lisp", "archive/v4/slices/SLICE34.md"],
        c_files=["samples/bootstrap-v4-plan-contract-tick.lisp", "v4/DECISION.md", "v4/LISP-ONLY.md"],
        b_test="samples/bootstrap-v4-wave86-runner-tick.lisp",
        c_test="archive/v4/slices/SLICE34.md",
    ),
    135: dict(
        title="emit-ir-main",
        a=113,
        b_track="emit",
        c_track="irtable",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/v4-ir-words-v2.txt", "archive/v4/slices/SLICE44.md"],
        c_files=["samples/v4-ir-table-v1.lisp", "samples/bootstrap-v4-wave56-irtable-tick.lisp", "archive/v4/slices/SLICE56.md"],
        b_test="samples/v4-ir-words-v2.txt",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    136: dict(
        title="codegen-gates-main",
        a=114,
        b_track="codegen",
        c_track="gates",
        b_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/bootstrap-v4-build-gates-plan.lisp", "archive/v4/slices/SLICE31.md"],
        c_files=["samples/bootstrap-v4-wave69-gates-tick.lisp", "archive/v4/slices/SLICE31.md", "v4/PROGRESS.md"],
        b_test="samples/bootstrap-v4-codegen-kickoff.lisp",
        c_test="v4/PROGRESS.md",
    ),
    137: dict(
        title="emit-terminal-main",
        a=115,
        b_track="emit",
        c_track="codegen",
        extra_grep=["aarch64.emit.add.operands="],
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-codegen-kickoff.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    138: dict(
        title="runner-lisp-main",
        a=116,
        b_track="runner",
        c_track="lisponly",
        b_files=["samples/bootstrap-v4-wave95-runner-tick.lisp", "v4/LISP-ONLY.md", "v4/DECISION.md"],
        c_files=["samples/bootstrap-v4-wave64-lisponly-tick.lisp", "v4/LISP-ONLY.md", "v4/DECISION.md"],
        b_test="v4/LISP-ONLY.md",
        c_test="v4/DECISION.md",
    ),
    139: dict(
        title="post-main-milestone",
        a=117,
        b_track="longrun",
        c_track="eval",
        b_files=["v4/longrun-state.json", "archive/v4/factory-docs/LONG-RUN-TODO.md", "archive/v4/factory-docs/DEV-AGENTS-TEAM.md"],
        c_files=["v4/EVAL.md", "v4/PROGRESS.md", "v4/REFLECTION.md"],
        b_test="v4/longrun-state.json",
        c_test="v4/EVAL.md",
    ),
    140: dict(
        title="emit-bytes-obs",
        a=118,
        b_track="emit",
        c_track="codegen",
        extra_grep=[
            "aarch64.emit.add.result=",
            "aarch64.emit.add.bytes=20",
        ],
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE113.md",
            "archive/v4/slices/SLICE140.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "archive/v4/slices/SLICE41.md",
        ],
        b_test="archive/v4/slices/SLICE113.md",
        c_test="archive/v4/slices/SLICE140.md",
    ),
    141: dict(
        title="runner-codegen-v5",
        a=119,
        b_track="runner",
        c_track="codegen",
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "samples/bootstrap-v4-wave95-runner-tick.lisp",
            "archive/v4/slices/SLICE86.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/bootstrap-v4-wave86-runner-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    142: dict(
        title="codegen-emit-v5",
        a=120,
        b_track="codegen",
        c_track="emit",
        extra_grep=["aarch64.emit.add.operands="],
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "archive/v4/slices/SLICE8.md",
            "archive/v4/slices/SLICE9.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE140.md",
            "archive/v4/slices/SLICE65.md",
        ],
        b_test="archive/v4/slices/SLICE8.md",
        c_test="archive/v4/slices/SLICE140.md",
    ),
    143: dict(
        title="runner-plan-v5",
        a=121,
        b_track="runner",
        c_track="plan",
        b_files=[
            "samples/bootstrap-v4-wave95-runner-tick.lisp",
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "archive/v4/slices/SLICE34.md",
        ],
        c_files=[
            "samples/bootstrap-v4-plan-contract-tick.lisp",
            "v4/DECISION.md",
            "v4/LISP-ONLY.md",
        ],
        b_test="samples/bootstrap-v4-wave95-runner-tick.lisp",
        c_test="archive/v4/slices/SLICE34.md",
    ),
    144: dict(
        title="emit-ir-v5",
        a=122,
        b_track="emit",
        c_track="irtable",
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "samples/v4-ir-words-v2.txt",
            "archive/v4/slices/SLICE44.md",
        ],
        c_files=[
            "samples/v4-ir-table-v1.lisp",
            "samples/bootstrap-v4-wave56-irtable-tick.lisp",
            "archive/v4/slices/SLICE56.md",
        ],
        b_test="samples/v4-ir-words-v2.txt",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    145: dict(
        title="codegen-gates-v5",
        a=123,
        b_track="codegen",
        c_track="gates",
        b_files=[
            "samples/bootstrap-v4-wave61-codegen-tick.lisp",
            "samples/bootstrap-v4-build-gates-plan.lisp",
            "archive/v4/slices/SLICE31.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave69-gates-tick.lisp",
            "v4/PROGRESS.md",
            "v4/MINDMAP.md",
        ],
        b_test="samples/bootstrap-v4-codegen-kickoff.lisp",
        c_test="v4/MINDMAP.md",
    ),
    146: dict(
        title="emit-lowering-v5",
        a=124,
        b_track="emit",
        c_track="codegen",
        extra_grep=["aarch64.emit.lowering.ops=5"],
        b_files=[
            "samples/bootstrap-v4-wave65-emit-tick.lisp",
            "archive/v4/slices/SLICE8.md",
            "archive/v4/slices/SLICE9.md",
        ],
        c_files=[
            "samples/bootstrap-v4-codegen-kickoff.lisp",
            "samples/v4-ir-table-v1.lisp",
            "archive/v4/slices/SLICE61.md",
        ],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="archive/v4/slices/SLICE8.md",
    ),
    147: dict(
        title="runner-lisp-v5",
        a=125,
        b_track="runner",
        c_track="lisponly",
        b_files=[
            "samples/bootstrap-v4-wave86-runner-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        c_files=[
            "samples/bootstrap-v4-wave64-lisponly-tick.lisp",
            "v4/LISP-ONLY.md",
            "v4/DECISION.md",
        ],
        b_test="v4/LISP-ONLY.md",
        c_test="v4/DECISION.md",
    ),
    148: dict(
        title="onion-milestone",
        a=126,
        b_track="onion",
        c_track="mindmap",
        b_files=[
            "v4/MINDMAP.md",
            "archive/v4/factory-docs/LONG-RUN-TODO.md",
            "v4/PROGRESS.md",
        ],
        c_files=[
            "v4/EVAL.md",
            "v4/REFLECTION.md",
            "archive/v4/factory-docs/DEV-AGENTS-TEAM.md",
        ],
        b_test="v4/MINDMAP.md",
        c_test="v4/EVAL.md",
    ),
    149: dict(
        title="emit-diffuse-a",
        a=127,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "archive/v4/slices/SLICE140.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/bootstrap-v4-codegen-kickoff.lisp", "archive/v4/slices/SLICE41.md"],
        b_test="archive/v4/slices/SLICE140.md",
        c_test="samples/bootstrap-v4-codegen-kickoff.lisp",
    ),
    150: dict(
        title="runner-diffuse-a",
        a=128,
        b_track="runner",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    151: dict(
        title="codegen-diffuse-a",
        a=129,
        extra_grep=['aarch64.emit.add.operands='],
        b_track="codegen",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    152: dict(
        title="runner-emit-bridge",
        a=130,
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    153: dict(
        title="codegen-ir-bridge",
        a=131,
        b_track="codegen",
        c_track="irtable",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    154: dict(
        title="emit-plan-bridge",
        a=132,
        b_track="emit",
        c_track="plan",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    155: dict(
        title="runner-codegen-bridge",
        a=133,
        b_track="runner",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    156: dict(
        title="emit-gates-bridge",
        a=134,
        extra_grep=['aarch64.emit.add.bytes=20'],
        b_track="emit",
        c_track="gates",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    157: dict(
        title="codegen-lisp-bridge",
        a=135,
        b_track="codegen",
        c_track="lisponly",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    158: dict(
        title="runner-diffuse-b",
        a=136,
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    159: dict(
        title="emit-diffuse-b",
        a=137,
        extra_grep=['aarch64.emit.lowering.ops=5'],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    160: dict(
        title="codegen-runner-b",
        a=138,
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    161: dict(
        title="emit-ir-b",
        a=139,
        b_track="emit",
        c_track="irtable",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    162: dict(
        title="runner-plan-b",
        a=140,
        b_track="runner",
        c_track="plan",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    163: dict(
        title="codegen-terminal-b",
        a=141,
        extra_grep=['aarch64.emit.add.result='],
        b_track="codegen",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    164: dict(
        title="emit-close-b",
        a=142,
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave86-runner-tick.lisp", "samples/v4-ir-table-v1.lisp", "archive/v4/slices/SLICE61.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="samples/v4-ir-table-v1.lisp",
    ),
    165: dict(
        title="diffuse-milestone",
        a=143,
        b_track="longrun",
        c_track="diffuse",
        b_files=["archive/v4/factory-docs/DIFFUSE-WORKFLOW.md", "v4/MINDMAP.md", "archive/v4/factory-docs/LONG-RUN-TODO.md"],
        c_files=["v4/EVAL.md", "v4/PROGRESS.md", "archive/v4/factory-docs/DEV-AGENTS-TEAM.md"],
        b_test="archive/v4/factory-docs/DIFFUSE-WORKFLOW.md",
        c_test="v4/EVAL.md",
    ),
    166: dict(
        title="onion-emit-a",
        a=144,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="emit",
        c_track="codegen",
        b_files=["archive/v4/slices/SLICE165.md", "samples/bootstrap-v4-wave65-emit-tick.lisp", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "archive/v4/slices/SLICE149.md", "archive/v4/slices/SLICE150.md"],
        b_test="archive/v4/slices/SLICE165.md",
        c_test="archive/v4/slices/SLICE149.md",
    ),
    167: dict(
        title="onion-runner-a",
        a=145,
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    168: dict(
        title="onion-codegen-a",
        a=146,
        extra_grep=['aarch64.emit.add.result='],
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    169: dict(
        title="onion-plan-a",
        a=147,
        b_track="plan",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    170: dict(
        title="onion-ir-a",
        a=148,
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    171: dict(
        title="onion-gates-a",
        a=149,
        extra_grep=['aarch64.emit.add.bytes=20'],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    172: dict(
        title="onion-lisp-a",
        a=150,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    173: dict(
        title="onion-emit-b",
        a=151,
        extra_grep=['aarch64.emit.lowering.ops=5'],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    174: dict(
        title="onion-runner-b",
        a=152,
        b_track="runner",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    175: dict(
        title="onion-codegen-b",
        a=153,
        extra_grep=['aarch64.emit.add.operands='],
        b_track="codegen",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    176: dict(
        title="onion-plan-b",
        a=154,
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    177: dict(
        title="onion-ir-b",
        a=155,
        b_track="emit",
        c_track="irtable",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    178: dict(
        title="onion-gates-b",
        a=156,
        b_track="codegen",
        c_track="gates",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    179: dict(
        title="onion-lisp-b",
        a=157,
        b_track="runner",
        c_track="lisponly",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    180: dict(
        title="onion-emit-c",
        a=158,
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    181: dict(
        title="onion-runner-c",
        a=159,
        extra_grep=['aarch64.emit.add.result='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "samples/bootstrap-v4-wave86-runner-tick.lisp", "archive/v4/slices/SLICE65.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/MINDMAP.md"],
        b_test="samples/bootstrap-v4-wave65-emit-tick.lisp",
        c_test="v4/MINDMAP.md",
    ),
    182: dict(
        title="onion-tdd-milestone",
        a=160,
        b_track="onion",
        c_track="mindmap",
        b_files=["v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md", "archive/v4/factory-docs/LONG-RUN-TODO.md"],
        c_files=["v4/EVAL.md", "v4/PROGRESS.md", "v4/REFLECTION.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/EVAL.md",
    ),
    183: dict(
        title="fast-emit-183",
        a=161,
        extra_grep=['aarch64.emit.add.result=', 'aarch64.emit.add.verified='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    184: dict(
        title="fast-runner-184",
        a=162,
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    185: dict(
        title="fast-codegen-185",
        a=163,
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    186: dict(
        title="fast-plan-186",
        a=164,
        extra_grep=['aarch64.emit.add.result='],
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    187: dict(
        title="fast-irtable-187",
        a=165,
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    188: dict(
        title="fast-gates-188",
        a=166,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    189: dict(
        title="fast-lisponly-189",
        a=167,
        extra_grep=['aarch64.emit.add.result='],
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    190: dict(
        title="fast-onion-190",
        a=168,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    191: dict(
        title="fast-emit-191",
        a=169,
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    192: dict(
        title="fast-runner-192",
        a=170,
        extra_grep=['aarch64.emit.add.result='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    193: dict(
        title="fast-codegen-193",
        a=171,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    194: dict(
        title="fast-plan-194",
        a=172,
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    195: dict(
        title="fast-irtable-195",
        a=173,
        extra_grep=['aarch64.emit.add.result='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    196: dict(
        title="fast-gates-196",
        a=174,
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    197: dict(
        title="fast-lisponly-197",
        a=175,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    198: dict(
        title="fast-onion-198",
        a=176,
        extra_grep=['aarch64.emit.add.result=', 'aarch64.emit.add.verified='],
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    199: dict(
        title="fast-emit-199",
        a=177,
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    200: dict(
        title="fast-runner-200",
        a=178,
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    201: dict(
        title="fast-codegen-201",
        a=179,
        extra_grep=['aarch64.emit.add.result='],
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    202: dict(
        title="fast-plan-202",
        a=180,
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    203: dict(
        title="fast-irtable-203",
        a=181,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    204: dict(
        title="fast-gates-204",
        a=182,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    205: dict(
        title="fast-lisponly-205",
        a=183,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    206: dict(
        title="fast-onion-206",
        a=184,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    207: dict(
        title="fast-emit-207",
        a=185,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    208: dict(
        title="fast-runner-208",
        a=186,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    209: dict(
        title="fast-codegen-209",
        a=187,
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    210: dict(
        title="fast-plan-210",
        a=188,
        extra_grep=['aarch64.emit.add.result='],
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    211: dict(
        title="cli-irtable-211",
        a=189,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    212: dict(
        title="cli-gates-212",
        a=190,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    213: dict(
        title="cli-lisponly-213",
        a=191,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    214: dict(
        title="cli-onion-214",
        a=192,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    215: dict(
        title="cli-emit-215",
        a=193,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    216: dict(
        title="cli-runner-216",
        a=194,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    217: dict(
        title="cli-codegen-217",
        a=195,
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    218: dict(
        title="cli-plan-218",
        a=196,
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    219: dict(
        title="cli-irtable-219",
        a=197,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    220: dict(
        title="cli-gates-220",
        a=198,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    221: dict(
        title="cli-lisponly-221",
        a=199,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    222: dict(
        title="cli-onion-222",
        a=200,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    223: dict(
        title="cli-emit-223",
        a=201,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    224: dict(
        title="cli-runner-224",
        a=202,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    225: dict(

        title="onion-225",
        a=203,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    226: dict(

        title="onion-226",
        a=204,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    227: dict(

        title="onion-227",
        a=205,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    228: dict(

        title="onion-228",
        a=206,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    229: dict(

        title="onion-229",
        a=207,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    230: dict(

        title="onion-230",
        a=208,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    231: dict(

        title="onion-231",
        a=209,
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    232: dict(

        title="onion-232",
        a=210,
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    233: dict(

        title="onion-233",
        a=211,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    234: dict(

        title="onion-234",
        a=212,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    235: dict(

        title="onion-235",
        a=213,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    236: dict(

        title="onion-236",
        a=214,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    237: dict(

        title="onion-237",
        a=215,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    238: dict(

        title="onion-238",
        a=216,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    239: dict(

        title="onion-239",
        a=217,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    240: dict(

        title="onion-240",
        a=218,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    241: dict(

        title="onion-241",
        a=219,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    242: dict(

        title="onion-242",
        a=220,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    243: dict(

        title="onion-243",
        a=221,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    244: dict(

        title="onion-244",
        a=222,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    245: dict(

        title="onion-245",
        a=223,
        b_track="codegen",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    246: dict(

        title="onion-246",
        a=224,
        b_track="plan",
        c_track="onion",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    247: dict(

        title="onion-247",
        a=225,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="irtable",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    248: dict(

        title="onion-248",
        a=226,
        extra_grep=['aarch64.emit.add.result='],
        b_track="gates",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    249: dict(

        title="onion-249",
        a=227,
        b_track="lisponly",
        c_track="runner",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    250: dict(

        title="onion-250",
        a=228,
        b_track="onion",
        c_track="mindmap",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    251: dict(

        title="onion-251",
        a=229,
        extra_grep=['aarch64.emit.add.result='],
        b_track="emit",
        c_track="codegen",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
    252: dict(

        title="onion-252",
        a=230,
        extra_grep=['aarch64.emit.add.verified='],
        b_track="runner",
        c_track="emit",
        b_files=["samples/bootstrap-v4-wave65-emit-tick.lisp", "v4/MINDMAP.md", "archive/v4/factory-docs/DIFFUSE-WORKFLOW.md"],
        c_files=["samples/bootstrap-v4-wave61-codegen-tick.lisp", "samples/v4-ir-table-v1.lisp", "v4/PROGRESS.md"],
        b_test="v4/MINDMAP.md",
        c_test="v4/PROGRESS.md",
    ),
}


def write_tick(wave: int, track: str, label: str, files: list[str]) -> str:
    name = f"bootstrap-v4-wave{wave}-{track}-tick.lisp"
    lines = [f"; wave{wave} track-{label}: {track}."]
    lines.append("(bootstrap")
    for i, f in enumerate(files):
        op = "file-hash" if i == len(files) - 1 else "file-size"
        lines.append(f'  ({op} "lab/nano-lisp-jit/{f}")')
    lines.append(")\n")
    (SAMPLES / name).write_text("\n".join(lines))
    return name


def gen_wave(wave: int, cfg: dict) -> dict:
    a = cfg["a"]
    expect = a + 17
    title = cfg["title"]
    (SAMPLES / f"nano-jit-slice-add-{expect}.lisp").write_text(
        f"; wave{wave}: add {a}+17={expect}.\n"
        "(module\n"
        "  (func add (param i64) (param i64) (load-arg-i64 0) (add-arg-i64 1))\n"
        f"  (main (i64 {a}) (save-top-i64) (i64 17) (call add) (expect {expect})))\n"
    )
    (SAMPLES / f"bootstrap-v4-wave{wave}-diffusion.lisp").write_text(
        f"; wave{wave} track-A: {title}.\n"
        "(bootstrap\n"
        '  (ir-table-lisp "lab/nano-lisp-jit/samples/v4-ir-table-v1.lisp")\n'
        f'  (build-slice-lisp "lab/nano-lisp-jit/samples/nano-jit-slice-add-{expect}.lisp"\n'
        f'                    "lab/nano-lisp-jit/.build/bootstrap-v4-slice{wave}-add{expect}.elf"\n'
        '                    "aarch64")\n'
        '  (squad-assess "lab/nano-lisp-jit/squad/catalog-v4.yaml")\n'
        '  (results-min "lab/nano-lisp-jit/.build/nano-jit/bootstrap-report.txt" "build.pass" "26")\n'
        '  (results-min "lab/nano-lisp-jit/.build/results.txt" "tests.pass" "270")\n'
        f'  (file-hash "lab/nano-lisp-jit/.build/bootstrap-v4-slice{wave}-add{expect}.elf"))\n'
    )
    b_file = write_tick(wave, cfg["b_track"], "B", cfg["b_files"])
    c_file = write_tick(wave, cfg["c_track"], "C", cfg["c_files"])
    (SAMPLES / f"bootstrap-v4-slice{wave}-evidence.lisp").write_text(
        f"; wave{wave} evidence.\n"
        "(bootstrap\n"
        f'  (file-size "lab/nano-lisp-jit/archive/v4/slices/SLICE{wave}.md")\n'
        f'  (file-hash "lab/nano-lisp-jit/.build/bootstrap-v4-slice{wave}-add{expect}.elf"))\n'
    )
    (ROOT / "v4" / f"SLICE{wave}.md").write_text(
        f"# v4 wave{wave} — {title}\n\n"
        "长程自主 · [`EVAL.md`](EVAL.md) · [`LONG-RUN-TODO.md`](LONG-RUN-TODO.md)。\n"
    )
    return dict(wave=wave, expect=expect, title=title, b_track=cfg["b_track"],
                c_track=cfg["c_track"], b_test=cfg["b_test"], c_test=cfg["c_test"],
                extra_grep=cfg.get("extra_grep", []))


def patch_bootstrap(expects: list[int]) -> None:
    import re

    c = Path("/workspace/lab/lispjit-ir/nano_bootstrap.c")
    t = c.read_text()
    last = max(expects)
    if f'add-{last})' in t:
        return
    nums = [int(x) for x in re.findall(r'strstr\(base, "add-(\d+)"\)', t)]
    if not nums:
        raise SystemExit("bootstrap add-N pattern not found")
    hi = max(nums)
    new_adds = [e for e in sorted(set(expects)) if e > hi]
    if not new_adds:
        return
    anchor = f'        strstr(base, "add-{hi}")) {{'
    if anchor not in t:
        raise SystemExit(f"bootstrap anchor add-{hi} not found")
    extra = " ||\n        ".join(f'strstr(base, "add-{e}")' for e in new_adds)
    t = t.replace(anchor, f'        strstr(base, "add-{hi}") ||\n        {extra}) {{')
    c.write_text(t)


def patch_run(meta: list[dict]) -> None:
    run = RUN.read_text()
    vars_lines = []
    cases = []
    for m in meta:
        w, e = m["wave"], m["expect"]
        bt, ct = m["b_track"], m["c_track"]
        vars_lines += [
            f'BOOTSTRAP_V4_WAVE{w}_DIFFUSION_SRC="$LAB_DIR/samples/bootstrap-v4-wave{w}-diffusion.lisp"',
            f'BOOTSTRAP_V4_WAVE{w}_{bt.upper()}_TICK_SRC="$LAB_DIR/samples/bootstrap-v4-wave{w}-{bt}-tick.lisp"',
            f'BOOTSTRAP_V4_WAVE{w}_{ct.upper()}_TICK_SRC="$LAB_DIR/samples/bootstrap-v4-wave{w}-{ct}-tick.lisp"',
            f'BOOTSTRAP_V4_SLICE{w}_EVIDENCE_SRC="$LAB_DIR/samples/bootstrap-v4-slice{w}-evidence.lisp"',
            f'V4_SLICE{w}_ADD{e}_ELF="$BUILD_DIR/bootstrap-v4-slice{w}-add{e}.elf"',
            f'V4_SLICE{w}_EVIDENCE="$BUILD_DIR/v4-slice{w}.evidence"',
        ]
        btest, ctest = m["b_test"], m["c_test"]
        extra = ""
        for pat in m.get("extra_grep", []):
            extra += f'\n  printf "%s\\n" "$out" | grep -q "{pat}"'
        cases.append(f'''
run_case "run-bootstrap-v4-wave{w}-diffusion-plan" bash -c '
  cd "'"$ROOT_DIR"'" && test -f "'"$BOOTSTRAP_REPORT"'"
  out=$("'"$RUNNER"'" run-bootstrap-plan "'"$BOOTSTRAP_V4_WAVE{w}_DIFFUSION_SRC"'" 2>&1) || true
  printf "%s\\n" "$out" | grep -q "aarch64.emit.ir.table.verified=plan-lisp-v1-full"
  test -f "'"$V4_SLICE{w}_ADD{e}_ELF"'"{extra}
'
run_case "run-bootstrap-v4-wave{w}-{bt}-tick-plan" bash -c '
  cd "'"$ROOT_DIR"'" && out=$("'"$RUNNER"'" run-bootstrap-plan "'"$BOOTSTRAP_V4_WAVE{w}_{bt.upper()}_TICK_SRC"'" 2>&1) || true
  test -f "'"$LAB_DIR"'/{btest}"
'
run_case "run-bootstrap-v4-wave{w}-{ct}-tick-plan" bash -c '
  cd "'"$ROOT_DIR"'" && out=$("'"$RUNNER"'" run-bootstrap-plan "'"$BOOTSTRAP_V4_WAVE{w}_{ct.upper()}_TICK_SRC"'" 2>&1) || true
  test -f "'"$LAB_DIR"'/{ctest}"
'
run_case "run-bootstrap-v4-slice{w}-evidence-plan" bash -c '
  cd "'"$ROOT_DIR"'" && out=$("'"$RUNNER"'" run-bootstrap-plan "'"$BOOTSTRAP_V4_SLICE{w}_EVIDENCE_SRC"'" 2>&1) || true
  test -f "'"$LAB_DIR"'/archive/v4/slices/SLICE{w}.md"
  {{ echo "v4.slice{w}=1"; }} >> "'"$V4_SLICE{w}_EVIDENCE"'"
'
''')
    block = "\n".join(vars_lines) + "\n"
    if f"V4_SLICE{meta[0]['wave']}_ADD" not in run:
        for anchor in (
            'V4_SLICE224_EVIDENCE="$BUILD_DIR/v4-slice224.evidence"\n',
            'V4_SLICE210_EVIDENCE="$BUILD_DIR/v4-slice210.evidence"\n',
            'V4_SLICE182_EVIDENCE="$BUILD_DIR/v4-slice182.evidence"\n',
            'V4_SLICE165_EVIDENCE="$BUILD_DIR/v4-slice165.evidence"\n',
            'V4_SLICE148_EVIDENCE="$BUILD_DIR/v4-slice148.evidence"\n',
            'V4_SLICE139_EVIDENCE="$BUILD_DIR/v4-slice139.evidence"\n',
            'V4_SLICE130_EVIDENCE="$BUILD_DIR/v4-slice130.evidence"\n',
            'V4_SLICE121_EVIDENCE="$BUILD_DIR/v4-slice121.evidence"\n',
            'V4_SLICE112_EVIDENCE="$BUILD_DIR/v4-slice112.evidence"\n',
            'V4_SLICE103_EVIDENCE="$BUILD_DIR/v4-slice103.evidence"\n',
            'V4_SLICE97_EVIDENCE="$BUILD_DIR/v4-slice97.evidence"\n',
            'V4_SLICE91_EVIDENCE="$BUILD_DIR/v4-slice91.evidence"\n',
            'V4_SLICE88_EVIDENCE="$BUILD_DIR/v4-slice88.evidence"\n',
            'V4_SLICE85_EVIDENCE="$BUILD_DIR/v4-slice85.evidence"\n',
            'V4_SLICE82_EVIDENCE="$BUILD_DIR/v4-slice82.evidence"\n',
            'V4_SLICE79_EVIDENCE="$BUILD_DIR/v4-slice79.evidence"\n',
            'V4_SLICE76_EVIDENCE="$BUILD_DIR/v4-slice76.evidence"\n',
            'V4_SLICE73_EVIDENCE="$BUILD_DIR/v4-slice73.evidence"\n',
            'V4_SLICE67_EVIDENCE="$BUILD_DIR/v4-slice67.evidence"\n',
        ):
            if anchor in run:
                run = run.replace(anchor, anchor + block)
                break
    ins = "".join(cases)
    if f"run-bootstrap-v4-wave{meta[0]['wave']}-diffusion-plan" not in run:
        for marker in (
            'run_case "run-bootstrap-v4-slice224-evidence-plan"',
            'run_case "run-bootstrap-v4-slice210-evidence-plan"',
            'run_case "run-bootstrap-v4-slice182-evidence-plan"',
            'run_case "run-bootstrap-v4-slice165-evidence-plan"',
            'run_case "run-bootstrap-v4-slice148-evidence-plan"',
            'run_case "run-bootstrap-v4-slice139-evidence-plan"',
            'run_case "run-bootstrap-v4-slice130-evidence-plan"',
            'run_case "run-bootstrap-v4-slice121-evidence-plan"',
            'run_case "run-bootstrap-v4-slice112-evidence-plan"',
            'run_case "run-bootstrap-v4-slice103-evidence-plan"',
            'run_case "run-bootstrap-v4-slice97-evidence-plan"',
            'run_case "run-bootstrap-v4-slice91-evidence-plan"',
            'run_case "run-bootstrap-v4-slice88-evidence-plan"',
            'run_case "run-bootstrap-v4-slice85-evidence-plan"',
            'run_case "run-bootstrap-v4-slice82-evidence-plan"',
            'run_case "run-bootstrap-v4-slice79-evidence-plan"',
            'run_case "run-bootstrap-v4-slice76-evidence-plan"',
            'run_case "run-bootstrap-v4-slice16-plan-words-plan"',
            'run_case "run-bootstrap-v4-slice73-evidence-plan"',
        ):
            if marker in run:
                run = run.replace(marker, ins + marker)
                break
    RUN.write_text(run)


RUN = ROOT / "run.sh"
CAT = ROOT / "squad" / "catalog-v4.yaml"
INDEX = SAMPLES / "v4-wave-index-v1.lisp"


def patch_catalog(meta: list[dict], section: str) -> None:
    t = CAT.read_text()
    if "id: v4-slice9-scoped" in t and "terminal_gates:" not in t:
        # slim catalog: wave batch gates live in run.sh only; still append tasks
        gates = []
    gates = []
    tasks = []
    for m in meta:
        w, e = m["wave"], m["expect"]
        bt, ct = m["b_track"], m["c_track"]
        gates += [
            f"    - id: v4-slice{w}-doc",
            f'      check: artifact',
            f'      path_glob: "archive/v4/slices/SLICE{w}.md"',
            f"    - id: v4-wave{w}-diffusion-plan",
            f'      check: artifact',
            f'      path_glob: "samples/bootstrap-v4-wave{w}-diffusion.lisp"',
            f"    - id: v4-slice{w}-add{e}-elf",
            f'      check: artifact',
            f'      path_glob: ".build/bootstrap-v4-slice{w}-add{e}.elf"',
            f"    - id: v4-wave{w}-{bt}-b-tick",
            f'      check: artifact',
            f'      path_glob: "samples/bootstrap-v4-wave{w}-{bt}-tick.lisp"',
            f"    - id: v4-wave{w}-{ct}-c-tick",
            f'      check: artifact',
            f'      path_glob: "samples/bootstrap-v4-wave{w}-{ct}-tick.lisp"',
            f"    - id: v4-slice{w}-evidence",
            f'      check: results',
            f'      file: ".build/v4-slice{w}.evidence"',
            f"      results_key: v4.slice{w}",
            f"      min_pass: 1",
            "",
        ]
        tasks += [
            f"  wave{w}-v4-track-a-codegen:",
            f"    assign_role: engineer-a",
            f"    priority: P0",
            f"    touch_paths:",
            f'      - "samples/bootstrap-v4-wave{w}-diffusion.lisp"',
            f'      - "samples/nano-jit-slice-add-{e}.lisp"',
            f'      - "run.sh"',
            f"    accept:",
            f'      - "run-bootstrap-v4-wave{w}-diffusion-plan"',
            f"  wave{w}-v4-track-b-{bt}:",
            f"    assign_role: engineer-b",
            f"    priority: P0",
            f"    touch_paths:",
            f'      - "samples/bootstrap-v4-wave{w}-{bt}-tick.lisp"',
            f'      - "run.sh"',
            f"    accept:",
            f'      - "run-bootstrap-v4-wave{w}-{bt}-tick-plan"',
            f"  wave{w}-v4-track-c-{ct}:",
            f"    assign_role: engineer-b",
            f"    priority: P0",
            f"    touch_paths:",
            f'      - "samples/bootstrap-v4-wave{w}-{ct}-tick.lisp"',
            f'      - "run.sh"',
            f"    accept:",
            f'      - "run-bootstrap-v4-wave{w}-{ct}-tick-plan"',
            f"  wave{w}-v4-track-d-evidence:",
            f"    assign_role: engineer-b",
            f"    priority: P0",
            f"    touch_paths:",
            f'      - "archive/v4/slices/SLICE{w}.md"',
            f'      - "run.sh"',
            f"    accept:",
            f'      - "run-bootstrap-v4-slice{w}-evidence-plan"',
            f"  wave{w}-v4-R:",
            f"    assign_role: reviewer",
            f"    priority: meta",
            f"    depends: [wave{w}-v4-track-a-codegen, wave{w}-v4-track-b-{bt}, wave{w}-v4-track-c-{ct}, wave{w}-v4-track-d-evidence]",
            f"    accept:",
            f'      - "v4/EVAL.md {section}"',
            f'      - "assess v4-complete ready"',
            "",
        ]
    gblock = "\n".join(gates)
    if f"v4-slice{meta[0]['wave']}-doc" not in t:
        t = t.replace("\n  terminal_gates:", "\n" + gblock + "\n  terminal_gates:")
    tblock = "\n".join(tasks)
    if f"wave{meta[0]['wave']}-v4-track-a-codegen" not in t:
        t = t.replace("\n\nverify:", "\n" + tblock + "\n\nverify:")
    CAT.write_text(t)


def patch_index(meta: list[dict]) -> None:
    t = INDEX.read_text()
    for m in meta:
        w, title = m["wave"], m["title"]
        line = f'  (wave {w} "{title}")'
        if line not in t:
            pos = t.rfind("\n)")
            if pos == -1:
                raise SystemExit("wave index closing paren not found")
            t = t[:pos] + "\n" + line + t[pos:]
    INDEX.write_text(t)


if __name__ == "__main__":
    lo = int(sys.argv[1]) if len(sys.argv) > 1 else 68
    hi = int(sys.argv[2]) if len(sys.argv) > 2 else 73
    meta = [gen_wave(w, WAVES[w]) for w in range(lo, hi + 1)]
    expects = [m["expect"] for m in meta]
    patch_bootstrap([e for e in expects if e > 62])
    patch_run(meta)
    section = f"wave{lo}–{hi}" if lo != hi else f"wave{lo}"
    patch_catalog(meta, section)
    patch_index(meta)
    print("ok", [m["wave"] for m in meta], "add", expects)
