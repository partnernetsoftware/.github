...
def wr16(buf: bytearray, off: int, val: int) -> None:
...
def find_symtab(data: bytes) -> tuple[int, int, int, int]:
    if data[:4] != b"\x7fELF":
        raise SystemExit("not ELF")
    shoff = rd64(data, 40)
    shentsize = rd16(data, 58)
    ...
    for i in range(1, shnum):
        ...
        if rd32(sh, 4) != 2:
            continue
        entsize = rd64(sh, 56)
        if entsize != ELF64_SYM_SIZE:
            raise SystemExit("bad symtab entsize")
        symtab_off = rd64(sh, 24)
        symtab_size = rd64(sh, 32)
        strtab_idx = rd32(sh, 40)
        if strtab_idx >= shnum:
            raise SystemExit("bad symtab link")
        str_sh = data[shoff + strtab_idx * shentsize : shoff + (strtab_idx + 1) * shentsize]
        strtab_off = rd64(str_sh, 24)
        strtab_size = rd64(str_sh, 32)
        sym_count = symtab_size // ELF64_SYM_SIZE
        return symtab_off, sym_count, strtab_off, strtab_size
    raise SystemExit("missing symtab")


def patch_symbol_shndx(data: bytearray, sym_name: str, new_shndx: int) -> None:
    symtab_off, sym_count, strtab_off, strtab_size = find_symtab(data)
    for idx in range(sym_count):
        sym_off = symtab_off + idx * ELF64_SYM_SIZE
        name_off = rd32(data, sym_off)
        name = section_name(data, strtab_off, strtab_size, name_off)
        if name == sym_name:
            wr16(data, sym_off + 6, new_shndx)
            return
    raise SystemExit(f"missing symbol {sym_name}")


def main() -> int:
    if len(sys.argv) != 5:
        print(
            f"usage: {sys.argv[0]} good.o bad-reloc-type.o bad-reloc-sym.o bad-symbol-shndx.o",
            file=sys.stderr,
        )
        return 1
    ...
    bad_symbol_shndx = bytearray(good)
    patch_symbol_shndx(bad_symbol_shndx, "nano_blob_data", 1)
    Path(sys.argv[4]).write_bytes(bad_symbol_shndx)
    return 0
