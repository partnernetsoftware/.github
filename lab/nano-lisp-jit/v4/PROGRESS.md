...
| Codegen | Lisp IR 整表 → blob | stub 读五 op `plan-lisp-v1-full` | **~47%** |
| 编排 | Lisp `(squad-*)` | assess + commander/resume 深链 | **~44%** |
| 构建 | plan 内 build 图 | wave79 add74 + results-min | **~56%** |
| 自举 | `.com` 生成下一代 | 未开卷 | **~0%** |

**整体终局（catalog ready 后继续自主）**：约 **15–22%**（wave77–79 见 [`EVAL.md`](EVAL.md) §wave77–79）（外圈证据满 ≠ 内圈替换完成）。
