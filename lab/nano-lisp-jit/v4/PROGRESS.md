...
| Codegen | Lisp IR 整表 → blob | stub 读五 op `plan-lisp-v1-full` | **~45%** |
| 编排 | Lisp `(squad-*)` | assess + commander/evidence-matrix | **~42%** |
| 构建 | plan 内 build 图 | wave31 add26 + results-min | **~54%** |
| 自举 | `.com` 生成下一代 | 未开卷 | **~0%** |

**整体终局（catalog ready 后继续自主）**：约 **15–22%**（wave68–73 见 [`EVAL.md`](EVAL.md) §wave31）（外圈证据满 ≠ 内圈替换完成）。
