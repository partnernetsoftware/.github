...
| **Pinned size** | **863 001 B** (~843 KiB; Wave 6 shell promote) | **2 959 413 B** (~2.82 MiB) |
...
Also pinned: `v45-selfhost-next.com` (**863 001 B**, manifest parity with `nano-lisp.com` after Wave 6) · bare `nanolisp.ape` (2 958 136 B) · **`nanolisp-slim.com`** (~161 KiB genesis-pin pathfinder).
...
| Track | Code | Notes |
|-------|------|-------|
| **C** | [`retired/archive-c/runner/`](../retired/archive-c/runner/) — `lispjit.c`, `nano_shell_cli.c`, … | Factory under `retired/archive-c/factory/`. **7b embed**: [`archive/c/embed/shell-script.lbin`](../archive/c/embed/shell-script.lbin) (280 B); release pin **863 001 B** after Wave 6 promote. |
...
|------|---------|------------|
| **Binary size** | **~843 KiB** (863 001 B pin; shell + factory slices) | ~2.8 MiB full CLI · **~161 KiB** `nanolisp-slim.com` genesis pathfinder |
...
| Need C wave SSOT COM with shell embed (~843 KiB pin) | Developing compiler, VM, APE pack, NLCap, bootstrap plans |
...
```bash
# C legacy (863001 B pin after Wave 6 shell promote)
...
| **Shell runner** | compile/run `shell-v0` via C COM; **c-gate** shell-noarg + fgets; no-arg **embedded** on 863 001 B pin | Ph **0–8** + **7 alt**; `nanolisp shell-full` rs-gate | C **Phase 8b**: `$COM run-bootstrap-plan …/bootstrap-v45-shell-full-c.lisp` — [`SHELL-RUNNER.md`](SHELL-RUNNER.md#phase-8--shell-full-bootstrap) |
