...
- `const-ptr` 是只读指针，`store-*` 会在 AOT/codegen 阶段以 `store-to-rodata` 拒绝；需要写入常量数据时使用显式 `mut-ptr`。
