...
路线图、v1 反思、v1.5/v2 洋葱 TDD mindmap 见 `ROADMAP.md`。
...
下一会话建议从 `ROADMAP.md` 的 `v1.5 / v2 洋葱 TDD mindmap` 继续，优先处理 v1.5 的 nano APE 格式：类似 Cosmopolitan，把跨架构 loader/识别头、多架构 payload table、架构选择逻辑和 manifest 检查做成 nano 自主格式；随后处理 `.rodata/.data` section、数据 relocation、`lispjit.c` 分层、bootstrap DSL build graph，以及 v2 的自托管 slice compiler 路径。
