...
2. 继续扩大 bootstrap DSL：在已落地的 `compile/hash/compare/run` 之外，继续接 `pack-app`、`inspect-app` 等可验证步骤。
...
- `bootstrap` 最小 DSL 已落地，可用 `.lisp` 顺序描述并执行 `compile` / `hash` / `compare` / `run` 子流程。
