...
## 地域范围（全球，不限中国）
- 检索与条目须覆盖 **全球**：北美、欧洲、亚太（含中、日、韩、印、东南亚等）、中东等，**不以中国或中文媒体为默认重心**。
- 中英文及其他语言源并重；优先一手官方与主流国际媒体（路透、彭博、TechCrunch、The Verge、各公司官方博客等）。
- 某区域动态仅在涉及 **新模型/新产品/新模式** 时收录；不要单独堆「某国产业评论」类条目。

## 采集
- 搜索词优先带：`release` `launch` `introducing` `new model` `open source` `workflow` `agent` `API` `beta` + 厂商/产品名；可辅以 `发布` `新模型` `开源`。
- 每条必须有可访问的 **出处 URL**；同一事件多源以 **官方稿** 为 SSOT，媒体稿只补独家细节。

## 输出格式（树状 Markdown）
1. 文首 **元数据表**：采集窗口、`yymmdd HH:MM` UTC 起止、更新时间、条目数、**新模型/新产品/新模式** 条数统计、main 合并 commit。
2. 用 ` ```text ` 画 **分类树**，顶层优先这三类，其余放后：
   `新模型` / `新产品` / `新模式` →（可选）资本动态 / 监管安全 / 区域要闻 / 综合周报
3. 每个分类下，每条资讯为一个 **二级列表项**，子字段固定顺序：
   - **标题**（原文标题）
   - **来源**：**来源缩写**（2–6 字，如 Anthropic、路透、TC、彭博）
...
| 类型 | 缩写示例 |
|---|---|
| 官方 | Anthropic、OpenAI、Google、Cursor、Meta、xAI |
| 国际媒体 | 路透、彭博、TC、Verge、TNW、WSJ |
| 区域媒体 | 按实际名缩短（中/日/韩/欧任意语言，不偏好某一国） |
...
| 260530 | 地域：明确全球覆盖，不限中国；分类树去掉「中国产业」默认项 |
