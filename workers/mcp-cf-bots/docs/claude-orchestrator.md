...
1. **首次**（你 Take Control 登录一次 `claude` 后告诉中台「已登录」）  
   → 中台：`python3 workers/mcp-cf-bots/tools/claude_code.py capture`（或 MCP `sess_put`）

2. **每次派活**  
   → 中台：`workers/mcp-cf-bots/scripts/claude_worker.sh -p "具体任务"`  
...
```bash
/workspace/workers/mcp-cf-bots/scripts/claude_worker.sh -p "加单元测试，不要改 API 行为"
```
