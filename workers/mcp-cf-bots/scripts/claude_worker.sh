#!/usr/bin/env bash
# Restore CLI auth from mcp-cf-bots, then run Claude Code.
set -euo pipefail
...
if [[ -z "${MCP_CF_BOTS_URL:-}" || -z "${MCP_CF_BOTS_TOKEN:-}" ]]; then
  echo "claude_worker: MCP_CF_BOTS_URL and MCP_CF_BOTS_TOKEN must be set" >&2
  exit 1
fi

python3 "$PKG/tools/claude_code.py" restore >/dev/null
if eval "$(python3 "$PKG/tools/claude_code.py" print-env 2>/dev/null)"; then
  :
fi
